...
<div class={`w-full overflow-x-auto py-4 ${className}`}>
	<ol class="flex items-start space-x-4 sm:space-x-6 px-4 min-w-max">
		{#each steps as step, i (step.id)}
			...
			<li class={`relative flex flex-col items-center ${i === steps.length - 1 ? '' : 'flex-1'}`}>
				...
				{#if i > 0}
					<div
						class={`absolute left-[-50%] top-[14px] -z-10 h-0.5 w-full ${baseConnectorBorderStyle} rtl:right-[-50%] rtl:left-auto`}
						aria-hidden="true">
						<!-- Garis solid jika langkah sebelumnya complete -->
						<div
							class={`h-0.5 w-full ${steps[i-1].status === 'complete' ? (theme === 'dark' ? 'bg-green-400' : 'bg-green-600') : 'bg-transparent'}`}></div>
					</div>
				{/if}

				<!-- Icon Container -->
				<div class="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-full shrink-0"
				...
				>
					<svelte:component
						...
						class={`w-4 h-4 sm:w-5 sm:h-5 ${getStatusColorClass(step.status)}`}
						strokeWidth={step.status === 'pending' ? 1.5 : 2}
					/>
				</div>

				<!-- Step Label & Description -->
				<div class="mt-2 text-center w-28 sm:w-32">
					<h3
						class={`text-xs sm:text-sm font-medium ${stepTextColor} ${step.status === 'current' ? 'font-semibold' : ''}`}>
						{step.label}
					</h3>
					{#if step.description}
