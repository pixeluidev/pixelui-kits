	...
	<div class="container mx-auto px-6">
		...
		<div class="mx-auto mb-10 max-w-2xl text-center md:mb-16">
			...
			<div class="mt-6 flex justify-center">
				<span class="inline-block h-1 w-24 rounded-full bg-orange-500"></span>
			</div>
		</div>
		...
		<div class="grid grid-cols-1 gap-6 md:gap-8 lg:grid-cols-3 xl:mt-12">
			{#each plans as plan (plan.id)}
				...
				<button
					...
					class:dark:border-orange-500={isSelected}
				...
				>
					...
					{#if plan.isRecommended}
						<div class="absolute -top-3 right-4">
							<span
								class="inline-flex items-center rounded-full bg-orange-500 px-3 py-1 text-xs font-semibold tracking-wide text-white shadow-md"
							>
								Recommended
							</span>
						</div>
					...
					<div class="mb-3 flex items-center gap-3 sm:mb-0">
						<svelte:component
							this={plan.icon}
							class={`h-6 w-6 flex-shrink-0 sm:h-7 sm:w-7 ${isSelected ? 'text-orange-600 dark:text-orange-500' : 'text-gray-400'}`}
							strokeWidth={2}
							aria-hidden="true"
						/>
		...
		{#if selectedPlan}
			...
			>
				<h3 class="mb-6 text-xl font-semibold text-gray-800 dark:text-white">
					Features included in <span class="text-orange-600 dark:text-orange-400"
				...
				<ul class="grid grid-cols-1 gap-x-8 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
					{#each selectedPlan.features as feature (feature.text)}
						...
						>
							{#if feature.included}
								<CheckCircle2
									class="h-5 w-5 flex-shrink-0 text-orange-500"
									strokeWidth={2.5}
									aria-hidden="true"
								/>
							{:else}
		...
		<div class="mt-10 flex justify-center md:mt-12">
			<button
				class="transform rounded-lg bg-orange-600 px-8 py-3 text-lg font-medium tracking-wide text-white capitalize transition-colors duration-300 hover:bg-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:ring-offset-gray-50 focus:outline-none dark:focus:ring-offset-gray-950"
			>
